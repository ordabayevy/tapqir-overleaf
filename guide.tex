...
\paragraph{Algorithm 3.} Variational distribution is provided in the form of a pseudocode of a stochastic function. In Pyro this is called the guide and is required to have the same structure as the model. Variational inference procedure is prone to get stuck in local maxima and is heavily affected by initial values of variational parameters. Below we provide the initial values of variational parameters that worked well in our analysis..

\algnewcommand{\Initialize}[1]{%
  \State \textbf{Variational parameter initializations and constraints:}
...
\Initialize{
    ...
    $\theta_\mathsf{prob} \gets 1/3^{\mathsf{target}[1+K] \times \mathsf{AOI}[N] \times \mathsf{frame}[F]}$, \quad $\sum_\mathsf{target} \theta_\mathsf{prob} = 1$ \\
    $m_\mathsf{prob} \gets \mathsf{target} \begin{array}[b]{@{}c@{}}\mathsf{spot}\\\begin{bmatrix} 0.5 & 0.5 \\ 1 & 0.5 \\ 0.5 & 1 \end{bmatrix}\end{array}^{\mathsf{AOI}[N] \times \mathsf{frame}[F]}$, \quad $[ 0, 1 ]$ \\
