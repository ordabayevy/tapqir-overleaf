...
\caption{\textbf{Tapqir analysis and inferred model parameters.} \textbf{a},\textbf{b}, Tapqir was applied to simulated data using the height3000 parameter set (Supplementary Data 1) (\textbf{a}) and the Rpb1\textsuperscript{SNAP549} experimental data set (Extended Data Table 1) (\textbf{b}). Each panel shows a short extract from a single target location in the dataset. The AOI images and corresponding schematics of Tapqir-detected spots are shown for the subset of frames indicated by gray shaded stripes in the plots. The first plot shows the probability of spot $k$ being present $p(m_{\mathsf{spot}(k)}=1)$, which is also shown as color intensity in the spot-detection row. The next two plots show the probability of spot $k$ being target-specific $p(\theta=k)$ and the probability of there being any target-specific spot in a frame $p(\mathsf{specific}) = p(\theta>0)$.  The bottom four plots show mean values (points) with estimated uncertainties (shading, $95\%$ CI) for the inferred integrated spot intensity $h$, spot position $x$ and $y$, and background intensity $b$. }
