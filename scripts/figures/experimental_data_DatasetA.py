...
im = ax.imshow(
    model.params["p(specific)"][: model.data.N][sdx][::3, ::2],
    norm=norm,
    aspect="equal",
    interpolation="none",
)
...
results = pd.read_csv("scripts/figures/DatasetA.csv", index_col=0)
# prepare data
Tmax = model.data.F
torch.manual_seed(0)
...
plt.savefig("figures/experimental_data_DatasetA.png", dpi=600)
